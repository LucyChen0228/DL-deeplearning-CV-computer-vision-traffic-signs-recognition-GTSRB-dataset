...
（1） 因为batch GD这种方法每更新一次参数都要把数据集里的所有样本都看一遍，计算量开销大，计算速度慢，不支持在线学；所以我采用了SGD算法：stochastic gradient descent，采用了非常非常小的学习率：0.001，而且由于现代的计算框架CPU/GPU的多线程工作，单个sample往往很难占满CPU/GPU的使用率，导致计算资源浪费，所以目前想要用：mini batch.
